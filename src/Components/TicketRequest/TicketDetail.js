...
export const TicketDetail = () => {
    ...
    useEffect(() => {
        const { startDate, endDate } = ticketDetailLocal;
        let newErrors = { ...errors };
        if (startDate && moment(startDate, ["DD/MM/YYYY"], true) < moment()) {
            newErrors = {
                ...newErrors,
                startDate: "Ngày Trong Quá Khứ"
            }
            return setErrors({...newErrors});
        } else {
            newErrors = omit({ ...newErrors }, ["startDate"]);
        }

        if (endDate && moment(endDate, ["DD/MM/YYYY"], true) < moment()) {
            newErrors = {
                ...newErrors,
                endDate: "Ngày Trong Quá Khứ"
            }
            return setErrors({...newErrors});
        } else {
            newErrors = omit({ ...newErrors }, ["endDate"]);
        }

        if(moment(startDate, ["DD/MM/YYYY"], true) > moment(endDate, ["DD/MM/YYYY"], true)){
            newErrors = {
                ...newErrors,
                startDate: "Ngày đi không hợp lệ",
                endDate: "Ngày về không hợp lệ"
            }
            return setErrors({...newErrors});
        }else{
            newErrors = omit({...newErrors}, ["startDate"]);
        ...
        setErrors({ ...newErrors });
    }, [ticketDetailLocal.startDate, ticketDetailLocal.endDate])
    return (
